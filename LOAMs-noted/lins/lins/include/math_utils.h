/* done
Description: calculatation function
01. sign
03. ensure -M_PI < x < M_PI
05. enforceSymmetry
07. axis(角轴) <-> quaternion
09. yaw pitch roll(欧拉角) <-> rotation matrix <-> quaternion
11. rad <-> deg
12. skew <-> positify
13. Qleft Qright Rleft Rinvleft
*/
...
{ // return x's sign(positive or negative number)
  // static_cast: convert (0) to data type <Derived>
  if (x >= static_cast<Derived>(0)) // force convert zero to type Derived
  {
    return 1;
  }
  else
...
template <typename Derived>
// ensure rotate in same direction; q=-q
