...
class ImageProjection
{
 private:
  ...
  cv::Mat labelMat;  // label matrix for segmentaiton marking: 0(no label)
  cv::Mat groundMat; // ground matrix for ground cloud marking: 1(ground)
  int labelCount;
 ...
 public:
  ...
  void allocateMemory()
  {
    ...
    // here are 4 directions: first 表示左右相邻 , second 表示上下相邻
  ...
  void findStartEndAngle()
  {
    ...
    segMsg.orientationDiff = segMsg.endOrientation - segMsg.startOrientation; // 终止与起始角度差
  }
  ...
  void projectPointCloud()
  {
    ...
    float verticalAngle, horizonAngle, range;   // 点的垂直角、水平角、距离
    size_t rowIdn, columnIdn, index, cloudSize; // 点的行号、列号、索引
    PointType thisPoint;
  ...
  void groundRemoval()
  {
    ...
    for (size_t j = 0; j < Horizon_SCAN; ++j)
    {
      ...
      } // endfor: find ground points in this column
  ...
  */
  void labelComponents(int row, int col)
  {
    ...
    // so here use array to replace queue and vector
    float d1; // 获取当前点和相邻点，距离较大值
    float d2; // 获取较小值
    float angle;
    float alpha; // 根据相邻方向获取水平或垂直方向的角度分辨率
    ...
    allPushedIndX[0]     = row;
    allPushedIndY[0]     = col;
    int allPushedIndSize = 1; // 和该点属于同一物体的点的个数
    ...
    while (queueSize > 0)
    {
      ...
      for (auto iter = neighborIterator.begin(); iter != neighborIterator.end(); ++iter)
      {
        ...
        { // 越大表明越平坦，表明为同一分类，添加进队列
          ...
          labelMat.at<int>(thisIndX, thisIndY) = labelCount; // 将其标记为同一分类
          lineCountFlag[thisIndX]              = true;       // 垂直方向的此行，已分类过

          allPushedIndX[allPushedIndSize] = thisIndX; // 是此分类的，均放入放入all pushed
          allPushedIndY[allPushedIndSize] = thisIndY;
          ++allPushedIndSize;
        }
      } // endfor: 处理了一个点的四个方向的相邻点

    } // endwhile: 为这个点找到了所有相邻点
