...
// 初始化控制变量
const int systemDelay = 20; // 弃用前20帧初始数据; cloud generated during initialization may be not healthy
int systemInitCount = 0;
bool systemInited = false;

// 激光雷达线数; num of row of cloud
...
// 点是否筛选过标志：0-未筛选过，1-筛选过; if 1, won't process this point any more
...
// imu循环队列长度; here we use array as a circular queue
const int imuQueLength = 200;

// 点云数据开始第一个点的位移/速度/欧拉角
float imuRollStart = 0, imuPitchStart = 0, imuYawStart = 0;
float imuRollCur = 0, imuPitchCur = 0, imuYawCur = 0; // Cur is current, 当前的
...
void AccumulateIMUShift()
{
  ...
  // 上一个imu点; we use % to make pointer can work as if this array is circular deque.
  int imuPointerBack = (imuPointerLast + imuQueLength - 1) % imuQueLength;
  // 上一个点到当前点所经历的时间，即计算imu测量周期 (测量周期可能不固定，每次需要计算)
  double timeDiff = imuTime[imuPointerLast] - imuTime[imuPointerBack];

  // For first frame, time diff will be a large value.
  // If not, means we have 2 imu frames at least and the "Back" makes sense.
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloudMsg)
{
  ...
  // 移除空点; sometimes the nan in cloud msg can be set as zero, they should be removed, too.
  ...
  { // 使用每个点的前后五个点计算曲率，因此前五个与最后五个点跳过(这个数字就是lego中的那个S集合的大小)
    ...
    // 初始化为less flat点（平面点远多于边缘点，这样设置的计算量更小）
    cloudLabel[i] = 0;
  ...
  { // 与后一个点差值，所以减6
    ...
    { // 前提:两个点之间距离要大于0.1???
      ...
      // 按照两点的深度的比例，将深度较大的点拉回后计算距离; so that we can calculate angle
      if (depth1 > depth2)
      {
  ...
  // publish IMU消息,由于循环到了最后，因此是Cur都是代表最后一个点，即最后一个点的欧拉角，畸变位移及一个点云周期增加的速度
...
void imuHandler(const sensor_msgs::Imu::ConstPtr &imuIn)
{
  ...
  tf::quaternionMsgToTF(imuIn->orientation, orientation); // 从imu中得到oriention
