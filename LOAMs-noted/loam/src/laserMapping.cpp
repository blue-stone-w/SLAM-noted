...
int laserCloudCenWidth = 10; // 意义不明？？？
int laserCloudCenHeight = 5;
int laserCloudCenDepth = 10;
const int laserCloudWidth = 21; // 意义不明？？？
...
void transformAssociateToMap()
{
  ...
  transformTobeMapped[1] = atan2(srycrx / cos(transformTobeMapped[0]),
                                 crycrx / cos(transformTobeMapped[0])); // 为什么有两个参数？？？
...
int main(int argc, char **argv)
{
  ...
  cv::Mat matV1(3, 3, CV_32F, cv::Scalar::all(0));  // eigen vector
  ...
  while (status)
  {
        ...
        fabs(timeLaserCloudFullRes - timeLaserOdometry) < 0.005)
    {
      ...
      if (frameCount >= stackFrameNum)
      {
          ...
          for (int j = centerCubeJ - 2; j <= centerCubeJ + 2; j++)
          {
            for (int k = centerCubeK - 2; k <= centerCubeK + 2; k++)
            {
              ...
              if (i >= 0 && i < laserCloudWidth && j >= 0 && j < laserCloudHeight && k >= 0 && k < laserCloudDepth)
              {
                ...
                } // endfor: have traversed 8 vertices of a cube
            ...
            } // endfor: traverse cubes in the direction z

          } // endfor: traverse cubes in the direction y

        } // endfor: traverse cubes in the direction x
        ...
        if (laserCloudCornerFromMapNum > 10 && laserCloudSurfFromMapNum > 100)
        {
          ...
          for (int iterCount = 0; iterCount < 10; iterCount++)
          {
            ...
            for (int i = 0; i < laserCloudCornerStackNum; i++)
            {
              ...
              if (pointSearchSqDis[4] < 1.0)
              {
                ...
                if (matD1.at<float>(0, 0) > 3 * matD1.at<float>(0, 1))
                {
                  ...
                  float a012 =
                      sqrt(((x0 - x1) * (y0 - y2) - (x0 - x2) * (y0 - y1)) * ((x0 - x1) * (y0 - y2) - (x0 - x2) * (y0 - y1)) +
                ...
                } // endif: corresponding line exists

              } // endif: if all close points are valid

            } // endfor: have traverse all edge points and add residual and weight
            ...
            for (int i = 0; i < laserCloudSurfStackNum; i++)
            {
              ...
              } // endif: all close points are valid
          ...
          } // endfor: iteration ends and get rotation and translation

          // 迭代结束更新相关的转移矩阵
          transformUpdate();
        } // endif: if there are enough points in map, match
        ...
        // 扭转量: transformation between frame and child frame
      ...
      } // endif: meet condition "跳帧" and process all data of this frame.

    } // endif: they are at the same time and are processed

    status = ros::ok();
    rate.sleep();
  } // end while: ros ends and exit loop
